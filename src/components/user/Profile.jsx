        ...
        try {
            if (file) {
                ...
                if (currentUser?.photoURL) {
                    const prevImage = currentUser.providerData[0].providerId === 'password' && currentUser?.photoURL?.split(`${currentUser?.uid}%2F`)[1].split('?')[0]
                    if (prevImage) {
                        try {
        ...
        !openCrop ? (
            <form onSubmit={handleSubmit}>
                <DialogContent dividers>
                    ...
                    <TextField autoFocus margin="normal" type='text' inputProps={{ minLength: 2 }} fullWidth variant="standard" value={name || ''} required onChange={(e) => setName(e.target.value)} />
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        <label htmlFor="profilePhoto">
                            <input type="file" accept="image/*" id="profilePhoto" style={{ display: "none" }} onChange={handleChange} />
